...
check_cb = keras.callbacks.ModelCheckpoint('checkpoints_2/' + file_name + '.{epoch:02d}-{val_loss:.2f}.hdf5',
                                           monitor='val_loss',
                                           verbose=0, period=10)
...
his = model.fit(X_train, y_train, validation_data=(X_test, y_test), batch_size=10, epochs=10, shuffle=True, callbacks=[check_cb, earlystop_cb])

pickle(his.history, open('history.pkl','wb'))
