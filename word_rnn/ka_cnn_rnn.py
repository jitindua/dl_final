...
import ka_data
X_train, y_train, X_test, y_test, W, W2 = ka_data.load_data(fold=0)
...
for i in xrange(folds):
    X_train, y_train, X_test, y_test, W, W2 = ka_data.load_data(fold=i)
    ...
    check_cb = keras.callbacks.ModelCheckpoint('checkpoints_ka_rnn/' + file_name + '.{epoch:02d}-{val_acc:.2f}-{val_loss:.2f}.hdf5', period = 10)
    tb_cb = keras.callbacks.TensorBoard(log_dir='./logs_ka_rnn', histogram_freq=10)
    ...
    for j in xrange(1):
        ...
        his = model.fit(X_train, y_train,
                        ...
                        shuffle=True, callbacks=[check_cb, tb_cb],
                        nb_epoch=nb_epoch, verbose=1)
        pickle.dump(his.hstory,open('his'+str(i)+'.pkl','wb'))
